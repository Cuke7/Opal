<template>
    <div class="p-2 bg-neutral rounded-3xl flex justify-between">
        <div class="flex items-center">
            ...
            <input @blur="saveNote" v-if="props.view" type="text" class="sm:hidden w-full mx-2 bg-transparent border-0 border-b-secondary border-b-2 px-2 font-mono text-lg text-neutral-conent" v-model="note.title" />
            <div v-else class="font-mono text-lg px-4 sm:hidden">
                {{ note.title }}
            </div>
            <input @blur="saveNote" type="text" class="hidden sm:flex w-full m-2 bg-transparent border-0 border-b-secondary border-b-2 font-mono text-lg text-neutral-content" v-model="note.title" />
        </div>
        <div class="dropdown dropdown-hover dropdown-end p-2" v-if="note.key">
            ...
            <ul tabindex="0" class="dropdown-content shadow rounded-box w-48 bg-neutral border-2 border-primary text-neutral-content space-y-3 flex flex-col py-4 px-4">
                <button class="flex justify-start items-center hover:bg-info hover:bg-opacity-50 p-2 rounded-lg" @click="saveNote()">
...
import { store, note, user, saveNote } from "../store";
import { DocumentArrowUpIcon, TrashIcon, EyeIcon, Bars3Icon, PencilIcon, EllipsisVerticalIcon } from "@heroicons/vue/24/outline";
